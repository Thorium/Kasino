...
let ``findExactSubsets should find two combos for target 7`` () =
    ...
    let results = Combinations.findExactSubsets 7 items
...
let ``findExactSubsets should find single item match`` () =
    let items = [ (0, 5); (1, 3) ]
    let results = Combinations.findExactSubsets 5 items
...
let ``findExactSubsets should return empty for no match`` () =
    let items = [ (0, 2); (1, 3) ]
    let results = Combinations.findExactSubsets 10 items
...
let private saako (target: int) (values: int list) =
    ...
    |> Combinations.findExactSubsets target
    |> List.isEmpty
    |> not
...
let ``findExactSubsets returns exact picks over a [1;2] table (ported YksKaks)`` () =
    ...
    Assert.Equal<int list list>([ [ 0 ] ], Combinations.findExactSubsets 1 table)
    // Target 2 -> just the Two.
    Assert.Equal<int list list>([ [ 1 ] ], Combinations.findExactSubsets 2 table)
    // Target 3 -> both cards together (1 + 2).
    Assert.Equal<int list list>([ [ 0; 1 ] ], Combinations.findExactSubsets 3 table)
